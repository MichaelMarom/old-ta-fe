...
const ShowCalendar = ({
  ...
  const getTutorSetup = async () => {
    const [result = []] = await get_tutor_setup({
      AcademyId: isStudentLoggedIn ? selectedTutor.academyId : tutorAcademyId,
    });
  ...
  const handleBulkEventCreate = async (type, invoiceNum) => {
    if (
      reservedSlots?.some((slot) => isEqualTwoObjectsRoot(slot, clickedSlot))
    ) {
  ...
  const handleSlotDoubleClick = (slotInfo) => {
    ...
    let endTime = secSlot
      ? moment(slotInfo.start).subtract(30, "minutes").toDate()
      : slotInfo.end;
    //student
    ...
    //some error handling
    let clickedUpperSlot =
      moment(slotInfo.end).diff(moment(slotInfo.start), "days") === 1;
    const isEventAlreadyExist = reservedSlots
      .concat(bookedSlots)
      ?.some(
        (event) =>
          convertToDate(event.start).getTime() === clickedDate.getTime() ||
          convertToDate(event.end).getTime() === slotInfo.end.getTime()
      );
    ...
    if (isEventAlreadyExist && slotInfo.action === "doubleClick") {
      toast.warning(
        "This time slot already reserved. Please select from white slots to continue!"
      );
      return;
    }
        ...
        if (disableWeekDays && disableWeekDays?.includes(dayName)) {
          if (activeView !== views.MONTH) {
            ...
            if (
              ...
              const removeEnableDate = enabledDays.filter(
                (date) =>
                  convertToDate(date).getTime() !== slotInfo.start.getTime()
              );
          ...
          if (activeView === views.MONTH) {
            const existInDisableDates = disableDates?.some(
              (date) =>
                convertToDate(date).getTime() === slotInfo.start.getTime()
            );
            const reservedSlotPresentInClickedDate = reservedSlots?.some(
              (slot) => moment(slot.start).date() === moment(clickedDate).date()
            );

            //disbaleDateRange
            const existInDisableDateRange = disableDateRange?.some(
            ...
            if (!existInDisableDateRange && !reservedSlotPresentInClickedDate)
              ...
              const removeDisableDateRange = disableDateRange.filter(
                (date) =>
                  convertToDate(date.start).getTime() !==
            ...
            if (!existInDisableDates && !reservedSlotPresentInClickedDate)
              ...
              const removeDisableDate = disableDates.filter(
                (date) =>
                  convertToDate(date).getTime() !== slotInfo.start.getTime()
              );
            ...
            if (
              ...
              const reservedSlotsHaveClickedSlot = reservedSlots?.some(
                (slot) =>
                  slot.start.getTime() ===
                  moment(slotInfo.start).startOf("hour").valueOf()
              );
              if (
                !disableHourSlots?.some(
                  (date) =>
  ...
  const slotPropGetter = useCallback(
    (date) => {
      if (date && moment(date).isSame(moment(date), "day")) {
        ...
        const existBetweenVacationRange =
          (isStudentLoggedIn
            ? selectedTutor.VacationMode
            : tutor.VacationMode) &&
        ...
        const existInSelectedSlotEnd = selectedSlots?.some(
          (slot) =>
            date.getTime() ===
            moment(slot.end).subtract(30, "minutes").toDate().getTime()
        );
        // tutor checks
        ...
        const existInDisableDates = disableDates?.some((slot) =>
  ...
  const eventPropGetter = (event) => {
    ...
    const deletedSession = reservedSlots
      .concat(bookedSlots)
      ?.some((slot) => slot.request === "delete" && slot.start === event.start);
    ...
    if (
      otherStudentSession &&
      (event.type === "intro" || event.type === "booked")
    ) {
      return {
        style: {
          border: "none",
