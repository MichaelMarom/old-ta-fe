...
const Accounting = () => {
  ...
  let [activeTab, setActiveTab] = useState(
    <TutorAccSetup
      sessions={sessions}
      currentYearAccHours={currentYearHrs}
      currentYearEarning={currentYearEarning}
      previousYearEarning={previousYearEarning}
    />
  );
  ...
  useEffect(() => {
    const fetchSessionDetails = async () => {
      ...
      const isLastYear = (date) =>
        getYear(date) === new Date().getFullYear() - 1;
      const currentYearSessions = sessions.filter((session) =>
        isCurrentYear(session.startDate)
      );
      const currentYearEarnings = currentYearSessions.reduce(
        (acc, session) => acc + session.rate,
        0
      );

      const lastYearSessions = sessions.filter((session) =>
        isLastYear(session.startDate)
      );
      const lastYearEarnings = lastYearSessions.reduce(
        (acc, session) => acc + session.rate,
        0
      );
