...
const GradePills = ({ grade, editable, setGrades, grades, hasIcon = true }) => {
    ...
    return (
        <div style={{
            ...
            className={`cursor-pointer m-2 rounded-pill p-2 d-flex align-items-center justify-content-between border shadow 
            ${!grades.find(item => item === grade) ? 'text-bg-transparent' : 'text-bg-success'}`}
