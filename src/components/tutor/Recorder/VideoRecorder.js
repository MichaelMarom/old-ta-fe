    ...
    try {
      if (data.size > 0) {
        ...
        previewRef.current.onloadedmetadata = () => {
          ...
          previewRef.current.addEventListener("pause", () =>
            setIsPlayingPreview(false)
          );
          previewRef.current.addEventListener("play", () =>
            setIsPlayingPreview(true)
          );
        };
        ...
        return () => {
          ...
          previewRef.current.removeEventListener("pause", () =>
            setIsPlayingPreview(false)
          );
          previewRef.current.removeEventListener("play", () =>
            setIsPlayingPreview(true)
          );
        };
      } else {
    ...
    async (retry = true) => {
      ...
      try {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
          throw new Error(
            "Unable to access media devices. Please check your browser settings."
          );
        }
      ...
      {recordingSupported && !blob && (
        <Webcam
          ...
          onUserMediaError={() =>
            logError(
              new Error(
                "Failed to access a camera, please check your device and browser settings"
              )
            )
          }
        />
