...
function TutorCalenderSidebar({
  ...
  const handleCheckboxClick = (day, timeRange) => {
    if (activeTab === "month") {
      ...
      if (disabledHours?.some((range) => range[0] === timeRange[0])) {
        setDisabledHours(
          disabledHours.filter((range) => range[0] !== timeRange[0])
        );
      } else {
  ...
  const checkboxChecked = (timeRangeOne) => {
    return disabledHours?.some((range) => range[0] === timeRangeOne);
  };

  return (
    <div className="tab-content card h-100" style={{ overflowY: "auto" }}>
      <div className="highlight small p-2">
        {" "}
        To view your student's feedback, click on past due date lessons that has
        stars icon. To postpone, or cancel a lesson, click on a futor date
        session, and follow dialog.
      </div>
      <div className="d-flex ">
        <TAButton
          className={` w-50 ${activeTab === "month" ? "month-btn" : "week-btn"}`}
          style={{
            boxShadow: `${
              activeTab === "month" ? "5px 5px 10px rgba(0, 0, 0, 0.5)" : ""
            }`,
          }}
          buttonText={"Blocked Days"}
          handleClick={() => handleTabClick("month")}
        />

        {/* <button
                    className={`action-btn btn w-50 ${activeTab === 'month' ? 'month-btn' : 'week-btn'}`}
                    style={{
                ...
                >
                 ...
                 <TAButton
          className={` w-50 ${activeTab === "day" ? "week-btn":"month-btn"}`}
          style={{
            boxShadow: `${
              activeTab === "day" ? "5px 5px 10px rgba(0, 0, 0, 0.5)" : ""
            }`,
          }}
          buttonText={"Blocked Hours"}
          handleClick={() => handleTabClick("day")}
        />
        {/* <button
          className={`action-btn btn w-50 ${
            activeTab === "day" ? "week-btn" : "month-btn"
          }`}
          style={{
            boxShadow: `${
              activeTab === "day" ? "5px 5px 10px rgba(0, 0, 0, 0.5)" : ""
      ...
      <div
        className={`h-100 tab-pane ${activeTab === "month" ? "active" : ""}`}
        id="months"
      >
        <div className="w-100 p-2">
          <label>
            Click on the pallet below to select a color indicating your blocked
            slots:
          </label>
        ...
        <div className="form-scheduling-cnt-left h-100 w-100">
          <div className="highlight small lh-sm">
            Checkbox the Day you are not tutoring. Students will not be able to
            book lessons for your blocked day(s).
          </div>
          ...
          <div className="form-scheduling-b-days">
            {days.map((day, index) => (
              <div className="form-check" key={index}>
                ...
                />
                <label
                  className="form-check-label small roboto-bold"
          ...
          <div className="highlight small lh-sm">
            Double click on a blocked day or hour, will unblock the day, or the
            specific hour for that day.
          </div>
      ...
      >
        <div className="form-scheduling-cnt-left flex-column d-flex  justify-content-between h-100 w-100">
          <div className="highlight small lh-sm">
            CheckBox the hours that you are not tutoring. Students will not be
            able to book lessons for your blocked hours. By default, Gray
            checkboxes are blooking undesirable hours. If desired, you can
            unblock any hour from this group to accomodate your student(s)
          </div>
              ...
              return (
                <div className="form-check" key={index}>
                  <input
                    ...
                    style={{
                      accentColor:
                        timeRange[2] !== "midnight" &&
                        checkboxChecked(timeRange[0], timeRange[1])
                          ? disableColor
                          : timeRange[2] === "midnight" &&
                            checkboxChecked(timeRange[0], timeRange[1])
                          ? "lightgray"
                    ...
                    className={`form-check-input border border-dark ${
                      timeRange[2] === "midnight" ? "gray-checkbox" : ""
                    }`}
